        ...
        <div class="container-fluid page__heading-container">
            <div class="page__heading d-flex align-items-center">
                <div class="flex">
                    ...
                    <h1 class="m-0">Lista das Permissões</h1>
                </div>
            </div>
        </div>
                                ...
                                <tbody class="list" id="staff02">
                                    {{-- @foreach ($permissao as $p)
                                        <tr>
                                            ...
                                            <td><span
                                                    class="js-lists-values-employee-name">{{ $p->rotas->nome }}</span>
                                            </td>
                                            <td>
                                    ...
                                    @foreach ($pe as $p)
                                        <tr>
                                            <td><strong><span class="js-lists-values-employee-name">{{ $p->nome }}</span></strong></td>
                                            @foreach($p->rotas as $r)
                                            <tr>
                                            <td><span
                                                    class="js-lists-values-employee-name">{{ $r->nome }}</span>
                                            </td>
                                            <td>
                                                <button wire:click.prevent="edit('{{ $r->pivot->id }}')"
                                                    class="btn btn-primary btn-sm">Alterar</button>
                                                <button wire:click.prevent="delete('{{ $r->pivot->id }}')"
                                                    class="btn btn-danger btn-sm">Apagar</button>
                                            </td>
                                            </tr>
                                            @endforeach
