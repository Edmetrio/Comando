<x-guest-layout>
    <x-auth-card>
        ...
        <form method="POST" action="{{ route('register') }}">
            ...
            <!-- Entidade -->
            <div class="mt-4">
                <x-input-label for="Entidade" :value="__('Entidade')" />
                <select id="small" name="entidade_id" :value="old('entidade_id')" required class="block p-2 mb-6 w-full text-sm text-gray-900 bg-gray-50 rounded-lg border border-gray-300 focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
                    <option selected>Selecciona a Entidade</option>
                    @foreach($entidade as $r)
            ...
            <div class="mt-4">
                ...
                <select id="small" name="role_id" :value="old('role_id')" required class="block p-2 mb-6 w-full text-sm text-gray-900 bg-gray-50 rounded-lg border border-gray-300 focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
                    <option selected>Selecciona a Role</option>
                    @foreach($role as $r)
                    <option value="{{$r->id}}">{{$r->nome}}</option>
                    @endforeach
                  </select>
